...
Source0:          http://www.eng.lsu.edu/mirrors/apache/thrift/%{version}/thrift-%{version}.tar.gz
...
sed -i -e 's/^AM_CPPFLAGS = /AM_CPPFLAGS = -DHAVE_NETINET_IN_H -DHAVE_INTTYPES_H /' \
  cpp/Makefile.am
...
pushd java/
sed -i \
	-e 's|"thrift_compiler" value="../../compiler/cpp/thrift"|"thrift_compiler" value="${thrift_home}/bin/thrift"|' \
	-e 's|"thrift_lib" value="../../lib/java/libthrift.jar"|"thrift_lib" value="${thrift_home}/share/java/libthrift.jar"|' \
	-e 's|../../lib/java/build/ivy/lib|/usr/share/java|' \
	build.xml
#-e 's|\(mkdir dir="${build.lib.dir}"/>\)$|\1\n    <mkdir dir="${build.tools.dir}"/>|' \
#	-e 's|${thrift.java.dir}/build/libthrift|${thrift_home}/share/java/libthrift|' \
#	-e 's|thrift_compiler" value="${thrift.root}/compiler/cpp/thrift"|thrift_compiler" value="${thrift_home}/bin/thrift"|' \
#	-e 's|${thrift.root}/lib/java/build/lib|${thrift_home}/share/java|' \
ant install \
	-Dthrift_home=%{_prefix} \
	-Ddist.dir=%{buildroot}%{_prefix} \
	-Ddist.lib.dir=%{buildroot}%{_javadir} \
	-lib %{_javadir} \
	-lib %{_javadir}/slf4j \
	-Dnoivy=
#%{__mkdir_p} %{buildroot}%{_javadir}
#%{__cp} build/libfb303-%{version}.jar %{buildroot}%{_javadir}/libfb303.jar
popd
%endif
